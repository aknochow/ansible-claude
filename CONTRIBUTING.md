# Contributing to ansible-claude
...
Ansible sanity tests require the repository to be within an `ansible_collections/aknochow/claude` directory hierarchy:
```bash
...
maps that layout into `site-packages/ansible_collections/aknochow/claude/`.
The PyPI name is `aknochow-claude` (not `ansible-claude` — Ansible trademark).
